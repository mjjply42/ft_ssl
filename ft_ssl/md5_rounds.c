...
t_mem	*padding(m_s *pre_image)
{
	...
	len = ft_strlen(((pre_image->STREAMS) ? pre_image->stream : pre_image->input));
	bitlen = len * 8;
	newlen = len + 1;
	while (newlen % 64 != 56)
		newlen++;
	...
	message->len = newlen;
	ft_memcpy(message->data, ((pre_image->STREAMS) ? pre_image->stream : pre_image->input), len);
...
void	md5_process(t_i *m, uint32_t *w, int i)
{
	...
	(*m).c = (*m).b;
	(*m).b = (*m).b +
		ROTATE_LEFT(((*m).a + (*m).f + g_k[i] + w[(*m).g]), g_r[i]);
	(*m).a = (*m).t;
}
